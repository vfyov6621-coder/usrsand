...
Команды:
  ...
  -ии on                     — режим диалога (отвечает на все сообщения)
  -ии off                    — выключить режим диалога
  -ии clear                  — очистить историю диалога
  -ии model <name>           — сменить модель
  -ии status                 — статус подключения
  -ии sys <текст>            — установить системный промпт
  -ии analyze @username [N]  — проанализировать последние N сообщений юзера
  -ии analyze reply [N]      — проанализировать последние N сообщений (reply)
  -ии summary [N]            — сводка по последним N сообщениям в чате
...
async def _handle_summary(client, message: Message, args_raw: str):
    """Обработать команду -ии summary — сводка по чату."""
    settings = _load_settings()
    model = settings.get("model", DEFAULT_MODEL)
...
def register(client):
    ...
    # -ии command handler — group 0 (default, runs first)
...
def on_load():
    _load_history()
    print(f"[AIChat] Loaded. -ии — Ollama at {OLLAMA_URL}, model: {DEFAULT_MODEL}")
